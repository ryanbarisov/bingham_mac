...
void vanka_smoother(dof_vector<double>& UVP, const dof_vector<double>& RHS0, int i, int j, int level)
{
	...
	for(int q = 0; q < 5; ++q)
	{
		if(puvp[q].inside()) RHS[q] = RHS0(puvp[q].grid-1, puvp[q])*scale;
		if(level == level0 && q < 4)
	...
	{
		D[0] = scale; U[0] = 0;
		if(level == level0) RHS[0] = scale*exact(test,1,puvp[0].x(),puvp[0].y());
		//else RHS[0] = 0.0;
	}
	else
	{
	...
	{
		D[1] = scale; U[1] = 0;
		if(level == level0) RHS[1] = scale*exact(test,1,puvp[1].x(),puvp[1].y());
		//else RHS[1] = 0.0;
	}
	else
	{
	...
	{
		D[2] = scale; U[2] = 0;
		if(level == level0) RHS[2] = scale*exact(test,2,puvp[2].x(),puvp[2].y());
	}
	else
	{
	...
	{
		D[3] = scale; U[3] = 0;
		if(level == level0) RHS[3] = scale*exact(test,2,puvp[3].x(),puvp[3].y());
		//else RHS[3] = 0.0;
	}
	else
	{
	...
	double alpha_uv = level == level0 ? 1.0 : 0.8;
	for(int q = 0; q < 4; ++q)
		D[q] /= alpha_uv;
...
void vanka_smoother(int iters, dof_vector<double>& UVP, const dof_vector<double>& RHS0, int level)
{
	...
	for(int iter = 0; iter < iters; ++iter)
	{
		...
		/*for(int i = 1; i < nx3+1; i+=2)
			for(int j = 1; j < ny3+1; j+=2)
				vanka_smoother(UVP, RHS0, i, j, level);
		for(int i = 2; i < nx3+1; i+=2)
			for(int j = 2; j < ny3+1; j+=2)
				vanka_smoother(UVP, RHS0, i, j, level);
		*/}
		else
		for(int i = nx3; i >= 1; --i)
...
void gs_smoother(dof_vector<double>& UVP, const dof_vector<double>& RHS0, int level, dof_vector<double>& EP, dof_vector<double>& EP0, int iter = 0)
{
	...
	int jU0 = 1, jU1 = ny1+1, djU = 1; if(swp) {jU0 = ny1; jU1 = 0; djU = -1;}
	int jV0 = 1, jV1 = ny2+1, djV = 1; if(swp) {jV0 = ny2; jV1 = 0; djV = -1;}
	int iV0 = 1, iV1 = nx2+1, diV = 1; if(swp) {iV0 = nx2; iV1 = 0; diV = -1;}
	int iP0 = 1, iP1 = nx3+1, diP = 1; if(swp) {iP0 = nx3; iP1 = 0; diP = -1;}
	int jP0 = 1, jP1 = ny3+1, djP = 1; if(swp) {jP0 = ny3; jP1 = 0; djP = -1;}
...
void fill_stokes(dof_vector<double>& RHS, multigrid_level* mgl, bool resid = false)
{
	...
	int nx3 = pint::nx(3,level), ny3 = pint::ny(3,level);
	double hx = pint::hx(level), hy = pint::hy(level);
	double scale = pint::hx(level0)*pint::hy(level0);
...
{
	int level = mgl->level;
	int nx1 = pint::nx(1,level+1), ny1 = pint::ny(1,level+1);
	int nx2 = pint::nx(2,level+1), ny2 = pint::ny(2,level+1);
	int nx3 = pint::nx(3,level+1), ny3 = pint::ny(3,level+1);
	static const double ucoef[2] = {1./8,1./4};
	// cycle over u-CV of coarse cells
	for(int iC = 1; iC < nx1+1; ++iC)
		for(int jC = 1; jC < ny1+1; ++jC)
	{
	...
	for(int jC = 1; jC < ny2+1; ++jC)
		for(int iC = 1; iC < nx2+1; ++iC)
	{
...
void gmg_stokes::prolongation(multigrid_level* mgl)
{
	...
	// cycle over u-CV of coarse cells
	for(int iC = 0; iC < nx1+1; ++iC)
		for(int jC = 0; jC < ny1+1; ++jC)
	{
		...
		if(dofC01.inside()) uC01 = U(0,dofC01);// else if(level == level0) uC01 = exact(test,1,dofC01.x(),dofC01.y());
		if(dofC02.inside()) uC02 = U(0,dofC02);// else if(level == level0) uC02 = exact(test,1,dofC02.x(),dofC02.y());
		if(dofC11.inside()) uC11 = U(0,dofC11);// else if(level == level0) uC11 = exact(test,1,dofC11.x(),dofC11.y());
		if(dofC12.inside()) uC12 = U(0,dofC12);// else if(level == level0) uC12 = exact(test,1,dofC12.x(),dofC12.y());
		if(dofC21.inside()) uC21 = U(0,dofC21);// else if(level == level0) uC21 = exact(test,1,dofC21.x(),dofC21.y());
		if(dofC22.inside()) uC22 = U(0,dofC22);// else if(level == level0) uC22 = exact(test,1,dofC22.x(),dofC22.y());
		// use du/dn = 0 near boundary
		if(iC == 0)	{uC01 = uC11; uC02 = uC12;}
		if(jC == 0)	{uC01 = uC02; uC11 = uC12; uC21 = uC22;}
		if(iC == nx1)	{uC21 = uC11; uC22 = uC12;}
		if(jC == ny1)	{uC02 = uC01; uC12 = uC11; uC22 = uC21;}
		int iF = 2*iC, jF = 2*jC;
		...
		//assert(dofF11.x() >= dofC11.x() && dofF11.y() >= dofC11.y() && dofF11.y() <= dofC12.y());
	...
	// cycle over v-CV of coarse cells
	for(int jC = 0; jC < ny2+1; ++jC)
		for(int iC = 0; iC < nx2+1; ++iC)
	{
		...
		if(dofC10.inside()) vC10 = U(1,dofC10);// else if(level == level0) vC10 = exact(test,2,dofC10.x(),dofC10.y());
		if(dofC20.inside()) vC20 = U(1,dofC20);// else if(level == level0) vC20 = exact(test,2,dofC20.x(),dofC20.y());
		if(dofC11.inside()) vC11 = U(1,dofC11);// else if(level == level0) vC11 = exact(test,2,dofC11.x(),dofC11.y());
		if(dofC21.inside()) vC21 = U(1,dofC21);// else if(level == level0) vC21 = exact(test,2,dofC21.x(),dofC21.y());
		if(dofC12.inside()) vC12 = U(1,dofC12);// else if(level == level0) vC12 = exact(test,2,dofC12.x(),dofC12.y());
		if(dofC22.inside()) vC22 = U(1,dofC22);// else if(level == level0) vC22 = exact(test,2,dofC22.x(),dofC22.y());
		// use du/dn = 0 near boundary
		if(iC == 0)	{vC10 = vC20; vC11 = vC21; vC12 = vC22;}
		if(jC == 0)	{vC10 = vC11; vC20 = vC21;}
		if(iC == nx2)	{vC20 = vC10; vC21 = vC11; vC22 = vC12;}
		if(jC == ny2)	{vC12 = vC11; vC22 = vC21;}
		int iF = 2*iC, jF = 2*jC;
